...
int open_port(char* port_name, int baud_rate)
{
	...
	{
		fprintf(stderr,"Serial port %s openend\n", port_name);
		fflush(stderr);
	}
	...
	CommParam.fDtrControl=DTR_CONTROL_ENABLE;	/* DTR Flow control                */
...
unsigned int write_to_serial(HANDLE	fd, unsigned char	*buffDati, unsigned int n_byte_to_write)
{
