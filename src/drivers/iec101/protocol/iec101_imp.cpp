...
void monitoring_dir_consumer(void* pParam)
{
	...
	iec101_imp* parent = (iec101_imp*)arg->parent;
	...
	while(1)
	{
		...
		for(int i = 0; (len = fifo_get(parent->fifo_monitor_direction, (char*)buf, sizeof(struct iec_item), wait_limit_ms)) >= 0; i += 1)	
		{ 
			//Receive in monitor direction as iec 101 packet
...
{
	iec101_imp * cl = (iec101_imp*)recvCallBackParam;
...
//  Class constructor.   
//   
iec101_imp::iec101_imp(struct iec101Context* my_ctx, char* line_number, int polling_time):
fExit(false),pollingTime(polling_time), is_connected(false),
exit_threads(0),n_msg_sent_monitor_dir(0),n_msg_sent_control_dir(0)
{   
	lineNumber = atoi(line_number);
		
	strcpy(my_iec101_context.serial_device, my_ctx->serial_device);
	my_iec101_context.baud = my_ctx->baud;
...
int iec101_imp::PollServer(void)
{
	IT_IT("iec101_imp::PollServer");
...
int iec101_imp::Start(void)
{
	IT_IT("iec101_imp::Start");
	
	char show_msg[200];
	sprintf(show_msg, " IndigoSCADA iec101 master Start\n");
	...
	int rc = iecserial_connect(my_iec101_context.serial_device, my_iec101_context.baud);
...
int iec101_imp::Stop()
{
	...
	char show_msg[200];
	sprintf(show_msg, " IndigoSCADA iec101 master End\n");
	LogMessage(NULL, show_msg);
...
void iec101_imp::LogMessage(int* error, const char* name)
{
...
void iec101_imp::get_local_host_time(struct cp56time2a* time)
{
	struct timeb tb;
	struct tm	*ptm;
...
void iec101_imp::get_utc_host_time(struct cp56time2a* time)
{
	struct timeb tb;
	struct tm	*ptm;
...
time_t iec101_imp::epoch_from_cp56time2a(const struct cp56time2a* time)
{
	struct tm	t;
	time_t epoch = 0;
	int ms;
...
short iec101_imp::rescale_value(double V, double Vmin, double Vmax, int* error)
{
	#ifdef DO_SCALE
	double Amin;
	double Amax;
...
double iec101_imp::rescale_value_inv(double A, double Vmin, double Vmax, int* error)
{
	#ifdef DO_SCALE
	double Amin;
	double Amax;
...
void iec101_imp::check_for_commands(struct iec_item *queued_item)
{
	if(!fExit)
	{ 
			...
			|| queued_item->iec_type == C_BO_NA_1)
		{
			...
			switch(queued_item->iec_type)
			{
				...
				default:
				{
					...
					iec101_imp::LogMessage(0, show_msg);
...
void iec101_imp::alloc_command_resources(void)
{

}

void iec101_imp::free_command_resources(void)
{
