...
/////globals//////////////////////////////////////////////////////////////////////////////////
u_short gl_common_address_of_asdu = 0;
u_short gl_link_address;
u_short gl_master_polling_time_in_milliseconds;
u_short gl_slave_polling_time_in_milliseconds;
int gl_read_timeout_ms = 300;
...
fifo_h fifo_control_direction; //fifo in control direction: SCADA-------------> RTU DA server if s->type == IEC_SLAVE
fifo_h fifo_monitor_direction; //fifo in monitor direction: SCADA<------------- RTU

//////////////////State machines///////////////////////////////////////////
static int state_database = DB_STARTUP; //if we NEED to load the database in the iec 101 slave
...
int state_iec_101_link = LNK_IDLE;
...
//#define USE_SELECT_BEFORE_OPERATE //comment this line if you do NOT use Select Before Operate
...
////////////////////iec buffers//////////////////////////////////////////////////////////////////////////
struct alloc_iec_buf v_iec_buf[IEC_101_MAX_EVENTS];
////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void get_iec_buf(struct iec_buf **c)
{
	...
	for(q_i = 0; q_i < IEC_101_MAX_EVENTS; q_i++)
	{
		if(v_iec_buf[q_i].used == 0)
		{
			memset(v_iec_buf[q_i].c, 0x00, IEC101_BUF_LEN);
			*c = v_iec_buf[q_i].c;
...
void process_timer_send_frame(struct iecserial *s, void *arg)
{
	...
	if((s->type == IEC_MASTER) && (state_iec_101_link != LNK_CONNECTED))
    {
   		//Invio lost link
		struct iec_item item_to_send;
		memset(&item_to_send,0x00, sizeof(struct iec_item));
		item_to_send.iec_type = C_LO_ST_1; //lost link
	...
	switch(state_database)
	{
		...
		case LOAD_DATABASE:
		{
			...
			if(iec_items_table == NULL)
			{
				...
				if(db == NULL)
				{
					//Il database non � presente sul gateway
				...
				if(recordcnt < 2)
				{
					//Il database non � stato generato correttamente
	...
	switch(state_file_transfer)
	{
		...
		case FT_PREPARE_DIRECTORY:
		{
			...
			answer_unit.num = 1;				
			answer_unit.sq = 1;  //Questa � un'asdu paccata => si possono impaccare nel frame 126 solo ioa adiacenti!
			answer_unit.pn = 0;				
			answer_unit.ca = gl_common_address_of_asdu;
			answer_unit.t = 0;				
	...
	switch(state_general_interrogation)
	{
		...
		break;
		case GI_REQUEST_GI_TO_FIELD:
		{
			if(s->type == IEC_SLAVE)
			{
				//The SLAVE sends to opc_client.exe or iec101master.exe the message C_IC_NA_1 (general interrogation command)
				...
				state_general_interrogation = GI_REQUEST_GI_TO_FIELD_DONE;
			}
		}
		break;
		case GI_LOAD_POINTS:
			...
			if(s->type == IEC_MASTER)
			{
				if(pn == 0)//positive initialised
				{
					//////////////Invio clock sync to SLAVE
					{
						...
						if (len > IEC101_ASDU_MAX)
						{
							fprintf(stderr,"len > IEC101_ASDU_MAX at line %d in file %s", __LINE__, __FILE__);
							fflush(stderr);
...
exit(EXIT_FAILURE);
			...
			else if(s->type == IEC_MASTER)
			{
				...
				if(cause == 0x07) //activation confirmation
				{
					if(pn == 0) //positive
					{
						...
						{
							//send test command to superior SCADA
							struct iec_item item_to_send;
							memset(&item_to_send,0x00, sizeof(struct iec_item));
							item_to_send.iec_type = C_TS_TA_1;
							item_to_send.iec_obj.ioa = obj[i].ioa; //ioa risposta
		...
		case C_SC_NA_1://single command
		{
			...
			{
				//Here we are in the Master, here we receive the activation confirmation or activation termination
				switch(cause)
				{
					case 0x07://activation confirmation
					{
                        //Send activation confirmation to superior scada
					    struct iec_item item_to_send;
					    memset(&item_to_send,0x00, sizeof(struct iec_item));
					    item_to_send.iec_type = C_SC_NA_1;
                        item_to_send.cause = 0x07;
			...
			if(s->type == IEC_SLAVE)
			{
				...
				if(obj[i].o.type46.se == 0)
				{
					...
					if(delta < MAX_TIME_AFTER_SBO_IN_SECONDS)
					{
						...
						//fifo_put(fifo_control_direction_ae, buf, sizeof(struct iec_item));
...
//							fflush(stderr);
		...
		case C_IC_NA_1://general interrogation command
		{
			if(s->type == IEC_SLAVE)
			{
				#define SEND_HERE_GI_CONFIRMATION
				#ifdef SEND_HERE_GI_CONFIRMATION
				...
				state_general_interrogation = GI_REQUEST_GI_TO_FIELD;
			}
			else if(s->type == IEC_MASTER)
			{
				//Siamo nel Master, qui riceviamo le conferme o terminazioni dallo slave
		...
		case C_CS_NA_1://clock synchronisation command
		{
			...
			else if(s->type == IEC_MASTER)
			{
				...
				if(cause == 0x07) //activation confirmation
				{
                    //Send clock synchronization activation confirmation command back to superior scada
				    struct iec_item item_to_send;
					memset(&item_to_send,0x00, sizeof(struct iec_item));
...
exit(EXIT_FAILURE);
				...
				{
					//Invio exit process command a opc_client.exe
					struct iec_item item_to_send;
					memset(&item_to_send,0x00, sizeof(struct iec_item));
					item_to_send.iec_type = C_EX_IT_1; //Exit process command
                    item_to_send.checksum = clearCrc((unsigned char *)&item_to_send, sizeof(struct iec_item));					
					fifo_put(fifo_control_direction, (char *)&item_to_send, sizeof(struct iec_item));
					//////////////////////////////////////////////////////////////////////////////
				}
...
//					iec_items_table[(items+k)->hClient - 1].iec_obj.o.type1 = (items+k)->iec_obj.o.type1; // - 1 poiche l'indice di iec_items_table parte da 0
		...
		case C_SC_NA_1://single command
		{
			if(s->type == IEC_SLAVE)
			{
				...
				if(!items->is_neg) //positive execution of command <16> decimale in qu
				{
			...
			{
				//We are in the MASTER, we receive comand from SCADA operator
				if(state_iec_101_link == LNK_CONNECTED) //Do not buffer the command if link is down!
				{
					struct iec_type45 answer_type;
				...
				{
					//Command rejected
					fprintf(stderr,"Command is rejected due to link down\n");
					fflush(stderr);
				}
			}
		}
		...
		case C_IC_NA_1://general interrogation command
		{
			...
			{
				//Siamo nello SLAVE opc_client.exe ha finito di fare la GA
		...
		case C_RP_NA_1: //Reset process command
		{
			...
			else if(s->type == IEC_SLAVE)
			{
				...
				//iecserial_flush_queues(s);// remove of old packets apa+++ 22-10-2010; commented out on 05-09-2011
				...
				*/
				...
				state_database = LOAD_DATABASE; //apa +++ on 05-09-2011 only for Hadeed code
			}
		}
		break;
		case C_SC_TA_1: //single command with time stamp
		{
			...
			else if(s->type == IEC_MASTER)
			{
				...
				if((state_clock_synchronisation == CLK_STATIONS_SYNCRONIZED) && (state_iec_101_link == LNK_CONNECTED))
				{
					...
					//IEC 101 MASTER e IEC 101 SLAVE stations siano sincronizzate sull'orologio
				...
				{
					//Comando scartato
					fprintf(stderr,"Comando scartato: stazioni non sicronizzate o link down\n");
					fflush(stderr);
				}
			}
		}
		...
		case C_DC_NA_1:
		{
			...
			{
				//Siamo nel MASTER, ricevo comando da operatore
				if(state_iec_101_link == LNK_CONNECTED) //non bufferizzo il comando!
				{
					//Siamo nel MASTER, qui invio il comando, proveniente dall'operatore dello SCADA superiore, allo slave
					struct iec_type46 answer_type;
		...
		case C_BO_NA_1:
		{
			if(s->type == IEC_MASTER)
			{
				...
				if(state_iec_101_link == LNK_CONNECTED) //non bufferizzo il comando!
				{
					...
					//items->opc_server_item_id e type51.stcd (che e' il valore del comando) e' inizializzato dallo SCADA superiore (nella dll scada_autonomo del PG)
			...
			else if(s->type == IEC_SLAVE)
			{
				...
				if(!items->is_neg) //positive execution of command <16> decimale in cd
				{
					answer_unit.pn = 0; //positive termination
				}
				else
		...
		case C_BO_TA_1:
		{
			if(s->type == IEC_MASTER)
			{
				...
				if((state_clock_synchronisation == CLK_STATIONS_SYNCRONIZED) && (state_iec_101_link == LNK_CONNECTED))
				{
					...
					//items->opc_server_item_id e type64.stcd (che e' il valore del comando) e' inizializzato dallo SCADA superiore (nella dll scada_autonomo del PG)
					//in stcd hai 32 bit, ci puoi scrivere anche ACK per fare l'acknowlede degli allarmi
		...
		case C_TS_TA_1://test command with time stamp
		{
			...
			{
				//Siamo nel MASTER, questo test comand arriva dallo SCADA superiore (es. PG)
				int len;
				struct iec_unit_id answer_unit;
				struct iec_type107 answer_type;
				u_int ioa; //importante: ioa da 3 ottetti deve essere u_int e non u_short
...
*/
		...
		//Send all items
		{
			...
			for( ;end < items_loaded; )
			{
				...
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type1) + IOA_ADDRLEN));

					max_el = max_el - 2; //to avoid buffer overun
				}
				else if(current_type == M_DP_NA_1)
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type3) + IOA_ADDRLEN));

					max_el = max_el - 2; //to avoid buffer overun
				}
				else if(current_type == M_ME_NC_1)
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type13) + IOA_ADDRLEN));

					max_el = max_el - 2; //to avoid buffer overun
				}	
				else if(current_type == M_IT_NA_1)
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type15) + IOA_ADDRLEN));

					max_el = max_el - 2; //to avoid buffer overun
				}
				else if(current_type == M_SP_TB_1)
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type30) + IOA_ADDRLEN));

					max_el = max_el - 2; //to avoid buffer overun
				}
				else if(current_type == M_DP_TB_1)
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type31) + IOA_ADDRLEN));

					max_el = max_el - 2; //to avoid buffer overun
				}
				else if(current_type == M_ME_TF_1)
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type36) + IOA_ADDRLEN));

					max_el = max_el - 2; //to avoid buffer overun
				}	
				else if(current_type == M_IT_TB_1)
				{
					max_el = (int)((IEC101_ASDU_MAX - IEC_TYPEID_LEN - COM_ADDRLEN)/(sizeof(struct iec_type37) + IOA_ADDRLEN));
...
int load_config_ini(char* numero_linea)
{
	...
	char LinkAddress[80];
	char MasterPollingTimeInMilliseconds[80];
//	char SelectBeforeOperate[80];
	...
	ret = GetPrivateProfileString(pItem,"LinkAddress","",LinkAddress,79,pInitFile);
	ret = GetPrivateProfileString(pItem,"PollingTimeInMilliSeconds","",MasterPollingTimeInMilliseconds,79,pInitFile);
//	ret = GetPrivateProfileString(pItem,"SelectBeforeOperate","",SelectBeforeOperate,79,pInitFile);
	...
	fflush(stderr);
                  ...
                  gc = g_key_file_get_string(gfile, pItem, "LinkAddress", NULL);
                  strcat(LinkAddress, gc);
                  
                  gc = g_key_file_get_string(gfile, pItem, "PollingTimeInMilliSeconds", NULL);
                  strcat(MasterPollingTimeInMilliseconds, gc);                  
#endif
	...
	gl_master_polling_time_in_milliseconds = atoi(MasterPollingTimeInMilliseconds);
...
int alloc_queues(void)
{
	...
	{
		v_iec_buf[i].c = (struct iec_buf*)calloc(1, IEC101_BUF_LEN);

		if(v_iec_buf[i].c == NULL)
		{
			fprintf(stderr, "Failed to allocate iec 101 queues\n");
			fflush(stderr);
			IT_EXIT;
			return 1;
		}
...
#define WORKING_WITH_INDIGOSCADA
...
#ifdef WORKING_WITH_INDIGOSCADA
	state_clock_synchronisation = CLK_STATIONS_SYNCRONIZED;
#else
	state_clock_synchronisation = CLK_IDLE;
#endif
	state_iec_101_link = LNK_IDLE;
	state_of_slave = SLAVE_NOT_INITIALIZED;
	//////////////////////////////////////////////
}
