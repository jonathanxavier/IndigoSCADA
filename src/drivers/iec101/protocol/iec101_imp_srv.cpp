...
void control_dir_consumer(void* pParam)
{
	...
	while(1)
	{
		...
		{ 
			//Receive in control direction as iec 101 packet
			...
			//Send in control direction as iec 101 packet
...
//  Class constructor.   
//   
iec101_imp::iec101_imp(struct iec101Context* my_ctx):
fExit(false), is_connected(false),
exit_threads(0),n_msg_sent_monitor_dir(0),n_msg_sent_control_dir(0)
{   
	strcpy(my_iec101_context.slave_port_name, my_ctx->slave_port_name);
	my_iec101_context.baud_rate = my_ctx->baud_rate;
...
int iec101_imp::Start(void)
{
	...
	char show_msg[200];
	sprintf(show_msg, " IndigoSCADA iec101 slave Start\n");
	...
	iecserial_connect(my_iec101_context.slave_port_name, my_iec101_context.baud_rate);
...
int iec101_imp::Stop()
{
	...
	char show_msg[200];
	sprintf(show_msg, " IndigoSCADA iec101 slave End\n");
	LogMessage(NULL, show_msg);
