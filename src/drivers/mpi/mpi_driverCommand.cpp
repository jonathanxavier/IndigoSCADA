...
File: Mpi_driverCommand.cpp
Last generated: Mon May 22 17:14:04 2000
*********************************************************************/
#include "Mpi_driverCommand.h"
#include <qt.h>
#include "Mpi_driver.h"
#define Inherited Mpi_driverCommandData
Mpi_driverCommand::Mpi_driverCommand
(
QWidget* parent,
const char* name,
...
{
	setCaption(tr("Mpi_driver Command"));
...
void Mpi_driverCommand::Help()
{
	//
	// invoke the help viewer for the Mpi_driver
	// 
	QSHelp("Mpi_driver_command");
}

void Mpi_driverCommand::OkClicked()
{
...
void Mpi_driverCommand::QueryResponse (QObject *p, const QString &c, int id, QObject*caller) // handles database responses
{
	if(p != this) return;
	switch(id)
	{
