...
void Opcua_DriverThread::run()
{
	IT_IT("Opcua_DriverThread::run");

	IT_COMMENT("Opcua_DriverThread Running");

    ///////////////////start child process opcua_master.exe////////////////////
	char line_number[50];
	...
	if(((Opcua_driver_Instance*)Parent)->isProtocolLoggingEnabled)
	{
		...
		strcat(pCommandLine, "\\logs\\opcua");
	...
	UnitFail("OPCUA driver stopped");

    Done = true;
}


bool Opcua_DriverThread::StartProcess(char* pCommandLine, char* pWorkingDir)
{ 
...
/*
PROCESS_INFORMATION* Opcua_DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Opcua_DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Opcua_DriverThread::send_ack_to_child(int address, int data, char* pipeName)
{
...
int Opcua_DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
...
void Opcua_DriverThread::TerminateProtocol() // parent requests the thread close
{
	IT_IT("Opcua_DriverThread::TerminateProtocol");
...
void Opcua_DriverThread::TerminateChild()
{
	IT_IT("Opcua_DriverThread::TerminateChild");
