...
void modbus_master_Instance::Start() 
{
	IT_IT("modbus_master_Instance::Start");
...
*Returns:none
*/
void modbus_master_Instance::Stop() // stop everything under this driver's control
{	
	IT_IT("modbus_master_Instance::Stop");
...
*Returns:none
*/
void modbus_master_Instance::QueryResponse(QObject *p, const QString &c, int id, QObject*caller) // handles database responses
{
	IT_IT("modbus_master_Instance::QueryResponse");
...
//						Values[k].fSpotValue = a.toInt(); 
		...
		case tUnitProperties:
		{
			if(GetConfigureDb()->GetNumberResults() > 0)
			{
				...
				is >> Cfg.MODBUServerIPAddress; // IEC 104 server IP Address
...
*/
//Realtime method
void modbus_master_Instance::driverEvent(DriverEvent *p)
{
	IT_IT("modbus_master_Instance::driverEvent");
...
//Realtime method
bool modbus_master_Instance::expect(unsigned int cmd)
{
	IT_IT("modbus_master_Instance::expect");
...
//Realtime method
void modbus_master_Instance::removeTransaction()
{
	IT_IT("modbus_master_Instance::removeTransaction");
...
void modbus_master_Instance::Tick()
{
	IT_IT("modbus_master_Instance::Tick");
...
//Realtime method
bool modbus_master_Instance::event(QEvent *e)
{
	IT_IT("modbus_master_Instance::event");
...
bool modbus_master_Instance::Connect() 
{	
	IT_IT("modbus_master_Instance::Connect");
	//return 0 on fail
	//retunr 1 on success
/*
	if(pConnect) delete pConnect;
	
	pConnect = new modbus_master_DriverThread(this); // create the connection
...
*Returns:none
*/
bool  modbus_master_Instance::Disconnect()        
{
	IT_IT("modbus_master_Instance::Disconnect");
...
*/
//Realtime method
bool  modbus_master_Instance::DoExec(SendRecePacket *t)
{
	IT_IT("modbus_master_Instance::DoExec");
...
*Returns:none
*/
void modbus_master_Instance::Command(const QString & name, BYTE cmd, LPVOID lpPa, DWORD pa_length, DWORD ipindex) // process a command for a named unit 
{
	IT_IT("modbus_master_Instance::Command");
