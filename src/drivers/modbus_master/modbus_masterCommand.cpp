...
File: modbus_masterCommand.cpp
Last generated: Mon May 22 17:14:04 2000
*********************************************************************/
#include "modbus_masterCommand.h"
#include <qt.h>
#include "modbus_master.h"
#define Inherited modbus_masterCommandData
modbus_masterCommand::modbus_masterCommand
(
QWidget* parent,
const char* name,
...
{
	setCaption(tr("modbus_master Command"));
...
void modbus_masterCommand::Help()
{
	//
	// invoke the help viewer for the modbus_master
	// 
	QSHelp("modbus_master_command");
}

void modbus_masterCommand::OkClicked()
{
...
void modbus_masterCommand::QueryResponse (QObject *p, const QString &c, int id, QObject*caller) // handles database responses
{
	if(p != this) return;
	switch(id)
	{
