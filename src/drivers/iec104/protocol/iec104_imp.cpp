...
void monitoring_dir_consumer(void* pParam)
{
	...
	while(1)
	{
		...
		{ 
			//Receive in monitor direction as iec 104 packet
...
//  Class constructor.   
//   
iec104_imp::iec104_imp(struct iec104Context* my_ctx, char* line_number):
...
int iec104_imp::PollServer(void)
{
...
int iec104_imp::Start(void)
{
	...
	char show_msg[200];
	sprintf(show_msg, " IndigoSCADA iec104 master Start\n");
	...
	addr.sin_port = htons(n_iec104server_port);
	
	#ifndef WIN32
	if(inet_pton(AF_INET, my_iec104_context.iec104ServerAddress, &addr.sin_addr) > 0)
	{
		fflush(stderr);
	...
	#else
	addr.sin_addr.s_addr = inet_addr(my_iec104_context.iec104ServerAddress);

	if(addr.sin_addr.s_addr!= INADDR_NONE)
	{
		rc = iecsock_connect(&addr);
	}
	#endif
