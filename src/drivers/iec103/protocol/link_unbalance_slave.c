...
{   
	...
	{   
		 ...
		 if(!frame_error)   
		 {   
			 if((rxfunc==9 || rxfunc==0 || rxfunc==7)&&(rxPRM)&&(!FCV))   
			 {   
				 //Master Request/ RESPOND expected
...
void set_link_state(int state)
{
	...
	switch(state)
	{
		case STAM_MASTERLINKAVAIL: state_iec_103_link = LNK_CONNECTED; break;
		case STAM_WAITREQSTATUS: state_iec_103_link = LNK_IDLE; reset_state_machines(); break;
		default: break;
	}
}
