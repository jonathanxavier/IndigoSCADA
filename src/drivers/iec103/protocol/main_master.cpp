...
#define APPLICATION "iec103master.exe"
...
"Run time usage: %s\n \
	...
	-p rts_on_time_ms\n \
	-q rts_off_time_ms\n \
	-u measurand_min_in_engineering_units\n \
	-v measurand_max_in_engineering_units\n \
	-w disturbance_min_in_engineering_units\n \
	-z disturbance_max_in_engineering_units\n \
	-f log_file_name\n"
...
int main( int argc, char **argv )
{
	...
	char RTSOnTime[80];
	char RTSOffTime[80];
	char MeasurandMin[80];
	char MeasurandMax[80];
	char DisturbanceMin[80];
	char DisturbanceMax[80];
	...
	while( ( c = getopt ( argc, argv, "a:n:s:l:f:t:m:p:q:u:v:w:z:?" )) != EOF ) {
		switch ( c ) {
			case 'a' :
			...
			break;
			case 'u':
				strcpy(MeasurandMin, optarg);
			break;
			case 'v':
				strcpy(MeasurandMax, optarg);
			break;
			case 'w':
				strcpy(DisturbanceMin, optarg);
			break;
			case 'z':
				strcpy(DisturbanceMax, optarg);
			break;
			case 'f' :
			{
				struct tm *ptm;
	...
	if(strlen(MeasurandMin) > 0)
		measurand_min = atof(MeasurandMin);

	if(strlen(MeasurandMax) > 0)
		measurand_max = atof(MeasurandMax);

	if(strlen(DisturbanceMin) > 0)
		disturbance_min = atof(DisturbanceMin);

	if(strlen(DisturbanceMax) > 0)
		disturbance_max = atof(DisturbanceMax);

	if(!IsSingleInstance(line_number))
	{
		fprintf(stderr,"Another instance of iec103master for line %s is already running!", line_number);
		fflush(stderr);
		if(iec_protocol_log_stream)
			fclose(iec_protocol_log_stream);
		return EXIT_FAILURE;
	...
	#ifdef WIN32
	strcpy(NewConsoleTitle, "iec103master LINK ");
	...
	struct iec103Context my_ctx;
	
	//iec103 RTU
	strcpy(my_ctx.serial_device, serial_device);
	my_ctx.baud = atoi(bits_per_second);
	
	iec103_imp* po = new iec103_imp(&my_ctx, line_number, atoi(PollingTimeInMilliseconds));
