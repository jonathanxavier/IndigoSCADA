        ...
        switch (record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA)
        {
            ...
            case 0x02: /* 2 byte integer (16 bit) */
                ...
                if (vif == 0x6C)
                {
                    ...
                    *value_out_str_size = my_snprintf(*value_out_str, 11, "%04d-%02d-%02d",
                                                 (time.tm_year + 1900),
                                                 (time.tm_mon + 1),
                                                  time.tm_mday);
                    result = 0;
            ...
            case 0x04: /* 4 byte integer (32 bit) */
                    ...
                    ((record->drh.vib.vif == 0xFD) && (vife == 0x70)))
                {
                    ...
                    *value_out_str_size = my_snprintf(*value_out_str, 21, "%04d-%02d-%02dT%02d:%02d:%02dZ",
                                                 (time.tm_year + 1900),
                                                 (time.tm_mon + 1),
                                                  time.tm_mday,
                                                  time.tm_hour,
...
mbus_data_variable_xml_normalized(mbus_data_variable *data)
{
    ...
    if (data)
    {
        ...
        len += my_snprintf(&buff[len], buff_size - len, "%s", mbus_data_variable_header_xml(&(data->header)));
        ...
        for (record = data->record, i = 0; record; record = record->next, i++)
        {
            ...
            len += my_snprintf(&buff[len], buff_size - len, "    <DataRecord id=\"%zu\">\n", i);
            ...
            if (norm_record != NULL)
            {
                ...
                len += my_snprintf(&buff[len], buff_size - len, "        <StorageNumber>%ld</StorageNumber>\n", norm_record->storage_number);

                if (norm_record->tariff >= 0)
                {
                    len += my_snprintf(&buff[len], buff_size - len, "        <Tariff>%ld</Tariff>\n", norm_record->tariff);
                    len += my_snprintf(&buff[len], buff_size - len, "        <Device>%d</Device>\n", norm_record->device);
                }
                ...
                len += my_snprintf(&buff[len], buff_size - len, "        <Quantity>%s</Quantity>\n", str_encoded);


                if (norm_record->is_numeric)
                {
                    len += my_snprintf(&buff[len], buff_size - len, "        <Value>%f</Value>\n", norm_record->value.real_val);
                }
                else
                {
                    mbus_str_xml_encode(str_encoded, norm_record->value.str_val.value, sizeof(str_encoded));
                    len += my_snprintf(&buff[len], buff_size - len, "        <Value>%s</Value>\n", str_encoded);
                }
...
mbus_context_serial(const char *device)
{
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: failed to allocate memory for handle\n", __PRETTY_FUNCTION__);
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: failed to allocate memory for device\n", __PRETTY_FUNCTION__);
        mbus_error_str_set(error_str);
        free(serial_data);
        free(handle);
        return NULL;
...
mbus_context_tcp(const char *host, uint16_t port)
{
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: failed to allocate memory for host\n", __PRETTY_FUNCTION__);
        mbus_error_str_set(error_str);
        free(tcp_data);
        free(handle);
        return NULL;
    ...
    for (i = 0; i <= handle->max_search_retry; i++)
    {
        ...
        if (ret == MBUS_PROBE_SINGLE)
        {
            ...
            if (mbus_frame_type(&reply) == MBUS_FRAME_TYPE_LONG)
            {
                ...
                my_snprintf(matching_addr, 17, "%s", addr);
