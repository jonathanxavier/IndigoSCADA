...
mbus_tcp_connect(mbus_handle *handle)
{
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: failed to setup a socket.", __PRETTY_FUNCTION__);
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: unknown host: %s", __PRETTY_FUNCTION__, host);
        mbus_error_str_set(error_str);
        return -1;
    }
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: Failed to establish connection to %s:%d", __PRETTY_FUNCTION__, host, port);
        mbus_error_str_set(error_str);
        return -1;
    }
...
mbus_tcp_send_frame(mbus_handle *handle, mbus_frame *frame)
{
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: mbus_frame_pack failed\n", __PRETTY_FUNCTION__);
        mbus_error_str_set(error_str);
        return -1;
    }
    ...
    {
        my_snprintf(error_str, sizeof(error_str), "%s: Failed to write frame to socket (ret = %d)\n", __PRETTY_FUNCTION__, ret);
        mbus_error_str_set(error_str);
        return -1;
    }
...
#define SSIZE_MAX 2^31 - 1
#define EWOULDBLOCK	3406
#endif
...
//apa+++
size_t my_snprintf(char *buf, size_t size, const char *fmt, ...)
{
 int len;
 va_list arg;
 va_start(arg, fmt);
