				       ...
				       bool isRow )
{
	...
	case Object_TextProperty:
	    ...
	    str = translate( (const char *)value.asCString(), comment );
				...
				QObject *parent, QListViewItem *parentItem )
{
	...
	case Object_TextProperty:
	    ...
	    if ( name == "text" )
		texts << translate((const char *)value.asCString(), comment );
	    break;
	case Object_VariantProperty:
	    unpackCString( strings, in, name );
	    unpackVariant( strings, in, value );
				    ...
				    QMenuBar *menuBar )
{
    ...
    menuBar->insertItem( translate((const char *)text), popupMenu );
}
...
void QWidgetFactory::setProperty( QObject* widget, const QString &prop, const QDomElement &e )
{
    ...
    if ( e.tagName() == "string" ) {
	value = translate((const char *) value.asString().utf8(), comment.utf8() );
...
void QWidgetFactory::loadMenuBar( const QDomElement &e )
{
	...
	if ( n.tagName() == "item" ) {
	    ...
	    mb->insertItem( translate( (const char *)n.attribute( "text" ).utf8() ), popup );
