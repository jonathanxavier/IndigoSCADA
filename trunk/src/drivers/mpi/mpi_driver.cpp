...
*Returns:none
*/
Mpi_driver::Mpi_driver(QObject *parent,const QString &name) : Driver(parent,name),n_iec_items(0)
{
...
*Returns:none
*/
void Mpi_driver::UnitConfigure(QWidget *parent, const QString &name, const QString &receipe) // configure a unit
{
	Mpi_driverConfiguration dlg(parent,name,receipe);
	dlg.exec();   
};
/*
*Function:UnitConfigure
...
*Returns:none
*/
void Mpi_driver::CommandDlg(QWidget *parent, const QString &name) // command dialog
{
	Mpi_driverCommand dlg(parent,name);
	dlg.exec();   
};
/*
...
*Returns:none
*/
void Mpi_driver::SetTypeList(QComboBox *pCombo, const QString &unitname) // set the type list for unit type
{
...
*Returns:none
*/
void Mpi_driver::GetInputList(const QString &type, QStringList &list,const QString &, const QString &) // set the permitted input IDs
{
/*
...
*Returns:none
*/
QWidget * Mpi_driver::GetSpecificConfig(QWidget *parent, const QString &spname, const QString &sptype) //specific config for sample point of type
{
	Mpi_driverInput * p;
	if(sptype == TYPE_M_ME_TC_1)
	{
		p = new Mpi_driverInput(parent,spname);
		return p;
	}
...
*Returns:none
*/
void Mpi_driver::GetTagList(const QString &type, QStringList &list,const QString &,const QString &) // returns the permitted tags for a given type for this unit
{
	if(type == TYPE_M_ME_TC_1)
	{
		list << VALUE_TAG;
...
*Returns:none
*/
void Mpi_driver::Command(const QString & instance,BYTE cmd, LPVOID lpPa, DWORD pa_length, DWORD ipindex)
{
	IT_IT("Mpi_driver::Command");
...
*Returns:none
*/
void Mpi_driver::CreateNewUnit(QWidget *parent, const QString &name, int n_inputs) // create a new unit - quick configure
{
...
//
Mpi_driver * Mpi_driver::pDriver = 0; // only one instance should be created
/*
*Function: Start
*Inputs:none
*Outputs:none
*Returns:none
*/
void Mpi_driver::Start() // start everything under this driver's control
{
		QString cmd = "select * from UNITS where UNITTYPE='mpi_driver' and NAME in(" + DriverInstance::FormUnitList()+ ");";
...
*Returns:none
*/
void Mpi_driver::Stop() // stop everything under this driver's control
{
...
*Returns:none
*/
void Mpi_driver::QueryResponse (QObject *p, const QString &, int id, QObject*caller)
{
	if(this != p) return;
	switch(id)
	{  
		case tListUnits:
		{
			...
			if(n > 0)
			{
				for(int i = 0; i < n; i++,GetConfigureDb()->FetchNext())
				{
					...
					Mpi_driver_Instance *p = new Mpi_driver_Instance(this, unit_name, i);
...
{ 
	#ifdef WIN32
	MPI_DRIVERDRV Driver *  _cdecl _GetDriverEntry(QObject *parent); 
	MPI_DRIVERDRV void _cdecl _Unload();
	#endif
	Driver *  _cdecl  _GetDriverEntry(QObject *parent) 
	{
		if(!Mpi_driver::pDriver)
		{
			Mpi_driver::pDriver = new Mpi_driver(parent,"Mpi_driver");
		};
		return Mpi_driver::pDriver;
	};
	/*
	*Function: _Unload
	...
	*/
	void _cdecl _Unload()
	{
		if(Mpi_driver::pDriver) delete Mpi_driver::pDriver;
		Mpi_driver::pDriver = 0;
	};
};
