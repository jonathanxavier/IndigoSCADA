...
void Mpi_DriverThread::run()
{
	IT_IT("Mpi_DriverThread::run");

	IT_COMMENT("Mpi_DriverThread Running");

    ///////////////////start child process mpi_master.exe////////////////////
	char line_number[50];
	...
	strcpy(pCommandLine, "C:\\scada\\bin\\mpi_master.exe -a ");
	...
	UnitFail("MPI driver stopped");

    Done = true;
}


bool Mpi_DriverThread::StartProcess(char* pCommandLine, char* pWorkingDir)
{ 
...
/*
PROCESS_INFORMATION* Mpi_DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Mpi_DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Mpi_DriverThread::send_ack_to_child(int address, int data, char* pipeName)
{
...
int Mpi_DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
...
void Mpi_DriverThread::TerminateIEC() // parent requests the thread close
{
	IT_IT("Mpi_DriverThread::TerminateIEC");
